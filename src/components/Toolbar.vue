...
import { getRecentFiles } from '../autosave/autosave'
...
const recentMenu = ref<HTMLDetailsElement | null>(null)
const recentFiles = ref<string[]>(getRecentFiles())

function onRecentMenuToggle() {
...
<template>
  <div class="toolbar">
    ...
    <details ref="recentMenu" class="import-menu" @toggle="onRecentMenuToggle">
      ...
      <div class="import-popover recent-popover">
        <span v-if="recentFiles.length === 0" class="recent-empty">No recent files</span>
        <span v-for="name in recentFiles" :key="name" class="recent-item" :title="name">{{ name }}</span>
      </div>
    </details>
